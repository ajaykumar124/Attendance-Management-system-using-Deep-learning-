...
def main(user_input, target_time_str, workbook, file_name):
    sheet = workbook.active
    ...
    if is_username_unique(sheet, user_input):
        ...
        current_date = datetime.now().strftime("%Y-%m-%d")
        sheet.append([user_input, current_date, current_time_str, result])
        workbook.save(file_name)
...
def start_stop_attendance_system():
    ...
    if not is_processing:
        ...
        workbook, _, file_name = create_or_open_workbook()
                ...
                if encodelist:  # Check if encodelist is not empty
                    for encodeface, facelocation in zip(curframe_encoding, face_locations):
                        ...
                        if results[match_index]:
                            ...
                            main(name, target_time_str, workbook, file_name)  # Pass the target time and attendance taking time to the main function
                            ...
                            cv2.putText(frame, "Unknown", (y1, x2 - 6),
...
bg_image = Image.open("bg3.jpg")  # Replace with your actual background image
